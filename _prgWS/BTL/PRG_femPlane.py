...
if __name__ == "__main__":
    ...
    GVL_integrationMode = "BBar"  # "SelReduced" / "BBar" / "Full" / "IncompatibleStrain"
    ...
    alpha1 = 2 * np.pi - np.deg2rad(alpha1)  # radians
    point_loads += pointLoadToXY(node=5, FF=F1, alphaF=alpha1)

    # Distributed loads:
    distributed_loads = []
    distributed_loads = [
        {'edge': (0, 1), 'direction': 1, 'value': (-1) * 2 * p},  # Load value (N/m) for nodes 1 and 2
        {'edge': (1, 2), 'direction': 1, 'value': (-1) * 2 * p},  # Load value (N/m) for nodes 2 and 3
        {'edge': (3, 7), 'direction': 0, 'value': (1) * p}   # Load value (N/m) for nodes 4 and 8
    ]
    
    # --- Apply Boundary Conditions ---
    fixed_dofs = []
    # Fix nodes 1, 5, 9, 10
    for fixed_node in [0, 4, 8, 9]:
        fixed_dofs.extend([2 * fixed_node, 2 * fixed_node + 1])
    # for fixed_node in [0, 2]:
    #     fixed_dofs.extend([2 * fixed_node, 2 * fixed_node + 1])
    ...
    # --- Plot Deformed mesh -----------------------------------------------------------------
    fig2 = plotDeformedMesh(nodes_global, elements, ux, uy, scaleFactor=20000, 
